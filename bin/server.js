const path = require('path');
const express = require('express');
const webpack = require('webpack');
const webpackConfig = require('../webpack.config');
const webpackDevMiddleware = require('webpack-dev-middleware');
const webpackHotMiddleware = require('webpack-hot-middleware');


const NODE_ENV = process.env.NODE_ENV || 'development';

const DIST_DIR = path.join(__dirname, '../dist');
const ROOT_DIR = path.join(__dirname, '..');

const PORT = process.env.PORT || 8080;


const app = express();
const compiler = webpack(webpackConfig);




function render(res, template, context = {}) {
    if(NODE_ENV === 'development') {
        compiler.outputFileSystem.readFile(path.join(DIST_DIR, 'templates', template), "utf8", (err, result) => {
            if (err) {
                return next(err);
            }
            res.set('content-type', 'text/html');
            res.send(result.replace(/(<%)(.*?)(%>)/g,''));
            res.end();
        });
    }
    else {
        res.render(template, context);
    }
}


if(NODE_ENV === 'development') {  
    app.use(webpackDevMiddleware(compiler, {
        publicPath: webpackConfig.output.publicPath
    }));

    app.use(webpackHotMiddleware(compiler));
}
else {
    app.set('view engine', 'ejs')
    app.set('views', path.join(DIST_DIR, 'templates'));
}

app.use('/static', express.static(path.join(DIST_DIR, 'static')));
app.use('/static', express.static(path.join(ROOT_DIR, 'static')));


app.get("/", (req, res) => {
    render(res, 'home.ejs', {head: '', content: ''});
});

app.get("/about", (req, res) => {
    render(res, 'about.ejs', {head: '', content: ''});
});



app.listen(PORT, ()=>{
    console.log(`listening to port ${PORT} (${NODE_ENV})`);
});